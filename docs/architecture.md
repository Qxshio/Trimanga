...
- `classifier`: converts OCR text plus page features into a conservative cleanup recommendation.
...
The second-pass visual matcher only uses high-confidence pages as seeds. That keeps repeated clutter pages detectable without letting weak OCR noise cascade into large false-positive runs.
